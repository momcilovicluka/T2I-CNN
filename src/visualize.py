...
# Loader->class-name cache for confusion-matrix tick labels. Loaders return

# (X, y, feature_names, class_names) with numeric labels aligned to

# class_names in order (verified in src/preprocessing.py: sklearn target

# order for breast, LabelEncoder.classes_ for dry bean, 0='<=50K' for adult).
...
def _class_names_for(dataset):

    if dataset not in _CLASS_NAMES_CACHE:

        sys.path.insert(0, str(Path(__file__).parent.parent))

        from src.preprocessing import (

            load_breast_cancer, load_dry_bean, load_adult_income,
        ...
        _, _, _, names = loaders[dataset]()

        _CLASS_NAMES_CACHE[dataset] = [str(n) for n in names]

    return _CLASS_NAMES_CACHE[dataset]
...
def plot_per_class_f1(results, output_dir='results/figures'):
    ...
    for idx, (method, data) in enumerate(best_per_method.items()):
        ...
        # FIX (2026-09-03, professor-validation 10.5D): labels reach sklearn
        # as INTEGERS (LabelEncoder in load_dry_bean), so classification_report
        # rows are "<class_id> <prec> <rec> <f1> <support>" and contain no
        # class NAME. The old parser searched for the class name in each line
        # and silently produced all-zero bars. Rows are indexed by numeric
        # class id, which matches class_names order (le.classes_).
...
def plot_confusion_matrices(results, output_dir='results/figures'):
    ...
    for idx, dataset in enumerate(DATASETS):
        ...
        # Class-name tick labels (numeric rows/cols are aligned with the

        # loader's class_names order for every dataset).

        names = _class_names_for(dataset)

        if len(names) == cm.shape[0]:

            rotate = 45 if len(names) > 2 else 0
...
def plot_runtime_comparison(results, output_dir='results/figures'):
    ...
    """Bar chart: total wall time per CNN cell, by T2I method × architecture.
    ...
    Uses total_time_sec when present (T2I fit+transform + CNN training,

    professor-validation 10.2) and falls back to train_time_sec for

    cells recorded before that field existed.
    ...
    def cell_time(r):
        ...
        tot = r.get('total_time_sec')

        if tot is not None:

            return tot

        return r.get('train_time_sec')



    cnn_results = [r for r in results if r.get('cnn_arch') in CNN_ARCHS

                   and cell_time(r) is not None]
    ...
    has_total = any(r.get('total_time_sec') is not None for r in cnn_results)
    ...
    fig.suptitle(('Total Wall Time by T2I Method and Architecture'

                   if has_total else

                   'Training Time by T2I Method and Architecture'),
